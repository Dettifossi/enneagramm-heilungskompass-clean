...
## Kriminalmusterkompass — Pflichtschritt bei jedem neuen Kriminalpsychologie-Porträt zu einem bereits bearbeiteten Subtyp

Die Rubrik „Kriminalmusterkompass" (`KRIMINALMUSTERKOMPASS`-Objekt in `bundle.js`, direkt nach `krankheitsmusterkompassDetailPage()`) funktioniert strukturell identisch zum Krankheitsmusterkompass, ist aber aus `KRIMINAL_PORTRAITS` gespeist. **Besondere Sensibilität, unbedingt beachten:** Diese Rubrik behauptet nie, dass ein Subtyp zu Kriminalität neigt — sie hält ausschließlich fest, welche **Form** eine seltene Entgleisung annahm, wenn eine der wenigen bereits porträtierten Personen eines Subtyps straffällig wurde. Jede neue oder überarbeitete `beschreibung`/`beleg` muss diesen Ton treffen: nie „ob", immer nur „wie, in den seltenen Fällen, in denen es passierte" — und nie eine Formulierung wählen, die suggeriert, ein Subtyp neige zu einer bestimmten Straftat.

Derselbe Pflichtschritt wie beim Krankheitsmusterkompass gilt bei jedem neuen Kriminalpsychologie-Porträt zu einem bereits bearbeiteten Subtyp: Fallzahl-Sprache prüfen/anpassen, neuen Fakt in einen passenden `beleg` einfügen, bei ausreichend neuen Fällen ein zweites Muster erwägen. Keine exakten Porträt-Zahlen in `beleg`/`beschreibung`. Die Beispielporträts-Liste zieht automatisch aus `KRIMINAL_PORTRAITS` (`kriminalmusterkompassPortraitsForCode()`).

## Berühmte Persönlichkeiten — die tatsächliche Größe/Leistung konkret einfangen, nicht nur behaupten

**Pflichtschritt bei jedem neuen Porträt in der Rubrik „Berühmte Persönlichkeiten".** Analog zum expliziten Wunsch bei Kriminalpsychologie-Porträts, mehr vom konkreten Tatgeschehen zu zeigen, gilt hier: Es reicht nicht, jemanden als „berühmtesten X seiner Zeit" zu bezeichnen und danach fast ausschließlich Herkunft, Fleiß/Disziplin und Enneagramm-Mechanik zu beschreiben. Ohne einen konkreten, fachlich substanziellen Absatz darüber, **was genau** diese Person außergewöhnlich machte, bleibt das Bild schief — man liest „arme Herkunft plus harte Arbeit" und fragt sich, warum ausgerechnet diese Person berühmt wurde, wenn unzählige andere Menschen ebenfalls diszipliniert hart arbeiten, ohne je berühmt zu werden.

**Konkret heißt das:** Bei jedem neuen Porträt aktiv recherchieren und mindestens einen eigenen Absatz einbauen, der die fachliche/künstlerische/sportliche/intellektuelle Substanz beschreibt, die diese Person tatsächlich einzigartig machte — z. B. bei einem Sänger die konkrete stimmliche Beschaffenheit (Timbre, Technik, Vergleich mit Zeitgenossen/Nachfolgern), bei einem Wissenschaftler die eigentliche fachliche Leistung, bei einem Sportler die konkrete technische oder taktische Besonderheit, bei einem Schriftsteller den stilistischen/inhaltlichen Beitrag. Diese Substanz darf gerne trotzdem im Enneagramm-Rahmen der SE3-„Können statt Glänzen"-Logik oder des jeweiligen Subtyps eingeordnet werden (z. B.: Talent wird durch Disziplin erst nutzbar gemacht) — aber die Substanz selbst muss zuerst konkret benannt sein, bevor sie eingeordnet wird. Talent und Begabung dürfen dabei ausdrücklich als real anerkannt werden, auch wenn eine porträtierte Person selbst (typbedingt) das eigene Talent herunterspielt und stattdessen nur die Arbeit betont — dieser Kontrast (reale Begabung vs. Selbstdarstellung als reine Leistung) ist selbst ein aufschlussreicher Teil der Typisierung, nicht etwas, das übernommen werden sollte, ohne es einzuordnen.
